...
class Profile():
    ...
    def evaluate_compatibility(self, compats: dict[str, ExprNode | None]):
        ...
        if self.category != 'printer':
            return
        for key, compat in compats.items():
            if not compat:
                self.compatible_profiles.append(key)
                continue
            if compat.eval(self.all_conf_dict):
                self.compatible_profiles.append(key)

    def generate_inherited_confs(self, all_confs_dict: dict[str, Any] | None = None):
        if all_confs_dict is None:
            all_confs_dict = {}
        self.all_conf_dict = generate_conf(all_confs_dict, self.key)
        if self.category == 'printer':
            nozzle_diam = str(self.all_conf_dict.get('nozzle_diameter', '0.4'))
            self.all_conf_dict['num_extruders'] = str(len(nozzle_diam.split(',')))
